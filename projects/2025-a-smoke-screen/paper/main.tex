...
\caption{五问共享的输入审计、机理内核、连续事件、组合决策与证据闭环。下方反向箭头表示验证失败时必须回到口径、机理或候选层修订。}
\label{fig:workflow}
\end{figure}
...
\caption{Q2 在速度 $140$ m/s、即时投放截面上的响应面与穿过连续解的两条正交剖面。星号和橙点为连续精化方案，浅色带给出不低于峰值 $95\%$ 的局部高值区。}
...
\]
标准差为 $1.1782\times10^{-13}$ s，四分位距为 $1.5987\times10^{-13}$ s，最大约束违反量为 0。该离散已处于双精度数值噪声量级，因此图\ref{fig:q2-multiseed}不把它放大成箱线分布，而是报告每代 best-so-far 距连续终值的差、中位数/IQR 和函数评估次数。结果说明在当前参数化和精化器下，8 个起点收敛到同一活动面；它仍不是解析的全局最优证书。

\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{q2_multiseed_stability.pdf}
\caption{Q2 八个独立随机种子的收敛缺口与计算代价。A 用灰色单次轨迹及中位数/IQR 取代对浮点噪声的箱线放大；B 给出每次函数评估量，并确认 $8/8$ 可行、终值极差为 $3.3\times10^{-13}$ s。}
...
\caption{Q3 三弹原始区间、逐弹端点、两处重叠与去重并集。图内算式显示单弹时长和扣除 $0.183$ s 重复覆盖后得到 $6.631$ s。}
\label{fig:q3-union}
\end{figure}
...
\caption{Q4 三架无人机的有效窗口、两个内部空窗与并集摘要。三个主窗口互不重叠，故联合目标等于三段时长之和。}
...
\caption{Q3、Q4 五个独立种子的局部精化轨迹与连续终值点图。灰线为单种子，色线/带为中位数/IQR；右侧不对 $n=5$ 使用箱线图，并直接报告范围与标准差。}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{search_quality_diagnostics.pdf}
\caption{Q3--Q5 的相对基线改进与搜索记账。A 将各问基线归一为 $100\%$ 以避免量级压制；B 用对数点图分列候选、航路与保留包，三者仅作计算规模记录而不解释为漏斗。}
...
\caption{Q5 空间策略的全局真比例总览与决策带纵向放大。图中同时给出无人机航迹、投放点、起爆点、导弹指派及同机弹序，颜色固定表示导弹。}
\label{fig:q5-xy}
\end{figure}
...
\caption{Q5 十五枚烟幕弹按导弹分组的有效区间与并集摘要行。标签中的弹序按每架无人机实际投放时刻生成；浅橙区标出各导弹内部最大空窗。}
\label{fig:q5-gantt}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.94\textwidth]{q5_per_missile_robustness.pdf}
\caption{Q5 同一策略在两种遮蔽口径下的配对敏感性审计。A 展示分导弹绝对损失和下降率，B 单列合计量，避免总量压缩分导弹差异。}
