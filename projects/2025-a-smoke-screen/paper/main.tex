...
问题一按题定策略解析计算投放点、起爆点和运动轨迹，以均匀扫描隔离候选边界，再用 Brent 法连续验根。主口径有效区间为 $[8.0130,9.4481]$ s，时长 $1.4351$ s；完整圆柱审计时长为 $1.3916$ s。问题二对航向、速度、投放时刻和引信延时联合优化，差分进化产生全局候选后作连续局部精化，得到航向 $6.9111^\circ$、速度 $140$ m/s、主口径遮蔽 $4.8325$ s。8 个独立随机种子的连续精化结果标准差为 $1.18\times10^{-13}$ s，最大约束违反量为 0。

问题三在同机固定航向、固定速度和投放间隔约束下，将三枚烟幕弹按新增并集长度组合；5 个独立种子均得到可行解，连续终值范围为 $5.6801$--$6.6311$ s，本批最好方案形成区间 $[4.8077,11.4388]$ s、总时长 $6.6311$ s。问题四通过时间多样性候选池协调三架无人机，5 个种子的终值范围为 $15.7366$--$15.7746$ s，本批最好方案的三个主要窗口互不重叠，总时长为 $15.7746$ s。问题五构造航路一致的同机投放包，再以有限状态搜索联合完成多机、多弹和多导弹指派；固定种子下得到一组经连续复算的可行解，对 M1、M2、M3 的主口径遮蔽并集分别为 $22.9078$、$16.3713$、$13.8049$ s，总计 $53.0839$ s，相同决策按完整圆柱口径复核后总计 $41.3551$ s。Q3 的种子离散不可忽略，Q5 尚未完成多种子审计，故均不作随机搜索稳定性强结论。

所有推荐方案的最大约束违反量为 0，连续边界最大距离残差不超过 $2.22\times10^{-11}$ m，结果表回算的最大坐标误差为 $5.46\times10^{-12}$ m。完整圆柱计算采用 96 个方位括区与连续精化，Q2 两个边界另以 23040 个方位节点交叉核验。本文明确区分“已找到根的精度”和“扫描括区的完备性”：固定步长变号扫描仍可能漏掉极窄正区间或切触，因而不把现有数值残差解释为全局完备性证明；Q3--Q5 亦只陈述经验证的可行解，尤其不宣称 Q5 全局最优。

\textbf{关键词：}烟幕干扰；有限视线；连续验根；区间并集；分层组合优化；保守审计
\end{abstract}
...
图\ref{fig:workflow}显示五问沿“解析运动--连续事件--区间并集--组合搜索--证据审计”逐层扩展，说明后一问复用了前一问已验证的几何与评分内核，而不是另起一套不一致模型。

\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{modeling_workflow.pdf}
\caption{五问共享的运动学、连续事件、区间并集、组合优化与验证流程。前一问的几何和区间模块直接成为后一问的评分内核。}
\label{fig:workflow}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{finite_sightline_geometry.pdf}
\caption{有限视线几何与烟幕截面的尺度关系。烟幕是否有效由球心到导弹--目标有限线段的距离决定。}
\label{fig:sightline-geometry}
\end{figure}

在时刻 $t$，导弹到目标中心的有限线段写为
\begin{equation}
\bm \ell_j(t,\lambda)=\bm m_j(t)+\lambda[\bm q-\bm m_j(t)],\qquad 0\leq\lambda\leq1.
\end{equation}
烟幕球心到该线段的最近投影系数、距离和遮蔽裕度为
\begin{align}
\lambda^*(t)&=\clip_{[0,1]}
\frac{[\bm c_{ik}(t)-\bm m_j(t)]^{\mathsf T}[\bm q-\bm m_j(t)]}
{\|\bm q-\bm m_j(t)\|^2},\label{eq:projection}\\
...
\label{eq:union}
\end{equation}
从而自动扣除同一导弹上的重叠时段。问题三、四最大化 $D_1$，问题五最大化
\begin{equation}
\max\;D_1+D_2+D_3.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{q1_occlusion_intervals.pdf}
\caption{Q1 两种遮蔽口径的连续裕度与有效区间。零水平线给出连续进入和退出边界；圆柱口径的进入时刻较晚。}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{q2_response_surface.pdf}
\caption{Q2 在速度 $140$ m/s、即时投放截面上的航向--起爆时刻响应面。星号为连续精化方案；零平台与狭窄高值区说明全局候选阶段是必要的。}
...
\begin{enumerate}[label=\arabic*.,leftmargin=2em]
  ...
  \item 对 8 个独立种子重复上述过程，报告最终值分布而非只报最好一次。
\end{enumerate}
\end{algobox}
...
8 次全局搜索全部成功。连续精化后的最小值、中位数和最大值为
\[
(T_{\min},T_{\mathrm{med}},T_{\max})
=(4.832501802462559,\ 4.8325018024627795,\ 4.832501802462888)\ \text{s}.
\]
标准差为 $1.1782\times10^{-13}$ s，四分位距为 $1.5987\times10^{-13}$ s，最大约束违反量为 0。图\ref{fig:q2-multiseed}同时报告每代 best-so-far 轨迹和连续复算终值分布。该结果说明在当前参数化和精化器下，8 个起点收敛到同一活动面；它仍不是解析的全局最优证书。

\begin{figure}[H]
\centering
\includegraphics[width=0.86\textwidth]{q2_multiseed_stability.pdf}
\caption{Q2 八个独立随机种子的逐代收敛轨迹与连续精化终值分布（$n=8$）。目标值离散处于双精度量级，所有运行约束违反量均为 0。}
...
\label{eq:q3-model}
\end{equation}
本问复用 Q2 的单弹连续区间函数，新增“同航路候选包”和“按新增并集长度计分”的组合层。

\subsection{算法披露}

路线网格共评估 1001 条航路，生成 1696 个对齐候选，保留 47 个候选包；连续重排后，再进行 320 次可行性保持的随机局部精化。为避免把固定种子误当稳定性，种子 20250808--20250812 各独立运行一次。本批所选种子 20250810 的 $0.04$ s 快速基线为 $3.6000$ s，局部精化前连续值为 $5.029085$ s，精化后粗评分为 $6.6415$ s、连续复算为 $6.631110248$ s。快速值只用于候选排序，连续值才进入论文和结果表。
...
\]
图\ref{fig:q3-union}直接标出三条原始区间和去重并集，说明约 $0.182914$ s 的单弹和来自重叠，不能重复计分。

\begin{figure}[H]
\centering
\includegraphics[width=0.86\textwidth]{q3_interval_union.pdf}
\caption{Q3 三弹原始有效区间与去重并集。并集而非单弹时长和定义本问目标。}
\label{fig:q3-union}
\end{figure}
...
\]
较主口径下降 $1.35\%$。三次投放的最小间隔为 $1.8666$ s；结果表回读后的最大坐标误差、最大单行时长误差和并集误差分别为 $3.64\times10^{-12}$ m、$9.05\times10^{-12}$ s 和 $3.85\times10^{-12}$ s，最大约束违反量为 0。五个种子的终值最小值、中位数、最大值和标准差分别为 $5.6801$、$6.4152$、$6.6311$ 和 $0.3936$ s，全部可行。其离散不可忽略，说明该局部搜索仍受种子影响；本文采用本批最好且经连续复算的可行解，但不把它称为稳定或全局最优。收敛轨迹与终值分布见图\ref{fig:q34-multiseed}。
...
三架无人机各投一弹，目标为
\begin{equation}
\max\ D_1=\meas(I_{111}\cup I_{211}\cup I_{311}).
\label{eq:q4-model}
\end{equation}
...
搜索共评估 1728 条航路，生成 1380 个单弹候选，按时间多样性保留 360 个候选，再进行 280 次可行性保持的随机局部精化。种子 20250808--20250812 各独立运行一次；本批所选种子 20250809 的 $0.04$ s 网格快速基线、局部精化前连续值、精化后粗评分和连续复算分别为 $15.7600$、$15.736598$、$15.8000$ 和 $15.774625$ s。快速计点和连续区间的评分口径不同，不能把粗值与连续值之差直接解释为真实性能变化；它们只记录粗筛到终稿复算的数值迁移。
...
\end{aligned}
\end{equation*}
三者互不重叠，总时长等于三个单弹时长之和，为 $15.774625$ s。图\ref{fig:q4-synergy}显示早、中、晚三个窗口的互补，而不是把“各自最长”当作“并集最长”。

\begin{figure}[H]
\centering
\includegraphics[width=0.86\textwidth]{q4_temporal_synergy.pdf}
\caption{Q4 三架无人机的有效窗口及时间互补关系。三个主窗口互不重叠，联合目标没有重复计分。}
\label{fig:q4-synergy}
\end{figure}

\subsection{本问检验与小结}

完整圆柱复核后总时长为 $10.430630$ s，下降 $33.88\%$。这一变化明显大于 Q3，原因是 FY3 的后期烟幕靠近地面，目标圆柱上下边缘对应的最不利视线更难同时进入半径 $10$ m 的烟幕球。结果表回读后的最大坐标误差、最大单行时长误差和目标误差分别为 $3.64\times10^{-12}$ m、$1.28\times10^{-12}$ s 和 $5.88\times10^{-13}$ s，最大约束违反量为 0。五个种子的终值范围为 $15.7366$--$15.7746$ s、标准差为 $0.0170$ s，全部可行；图\ref{fig:q34-multiseed}显示多数种子停在同一平台，仅种子 20250809 取得小幅改进。本问据此报告本批最好可行值，但仍没有全局最优证明。

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{q3_q4_multiseed_stability.pdf}
\caption{Q3、Q4 五个独立种子的随机局部精化轨迹与连续复算终值分布。Q3 离散明显，Q4 多数种子停在同一平台；所有运行的最大约束违反量均为 0。}
...
Q5 共评估 1440 条航路，生成 1334 个可用候选，最终在多机组合层保留 35 个路线包。$0.04$ s 中心视线网格快速基线为 $51.3700$ s，入选状态的快速评分为 $53.0600$ s，连续复算为 $53.083939170$ s。固定种子为 20250808。路线库和 35 个保留包均是有限的，且本问尚未完成多种子重复，因此该流程只提供一组可复现且经连续复算的可行解，没有给出搜索稳定性、松弛上界或全局最优性间隙。
...
图\ref{fig:search-quality}显示 Q3--Q5 的快速基线、粗评分、连续复算和搜索规模。Q3--Q5 的连续终值均高于各自快速基线，但两类评分口径不同，只用于记录候选发现到连续复算的数值迁移，不能据此计算严格改进率或最优性间隙。

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{search_quality_diagnostics.pdf}
\caption{Q3--Q5 的快速基线、候选粗评分、连续复算和搜索规模。不同评分层只用于展示数值迁移；连续复算值才是终稿值。}
\label{fig:search-quality}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{q5_xy_strategy.pdf}
\caption{Q5 无人机航向、投放点与起爆点的平面投影。颜色区分指派导弹；同机三弹沿同一水平航向运动。}
\label{fig:q5-xy}
\end{figure}
...
主口径总目标为 $53.083939$ s。这个总时长是三枚导弹各自并集长度之和；图\ref{fig:q5-gantt}展示逐弹有效区间及其合并关系。M1 的覆盖由多个时间段串联，M2、M3 仍有空档，因而该图同时给出继续搜索可能利用的位置，也说明当前结果不是上界。

\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{q5_coverage_gantt.pdf}
\caption{Q5 十五枚烟幕弹的有效区间、导弹归属与覆盖排程。同一导弹内部按并集计时，空白区域是当前方案未覆盖的时间段。}
\label{fig:q5-gantt}
\end{figure}

\subsection{本问检验与小结}

相同决策按完整圆柱口径复核后，三枚导弹的并集均低于中心视线结果。图\ref{fig:q5-robustness}显示 M1、M2、M3 的相对排序仍保持不变；判据变化显著影响绝对目标值，却没有改变当前方案的资源倾向。

\begin{figure}[H]
\centering
\includegraphics[width=0.78\textwidth]{q5_per_missile_robustness.pdf}
\caption{Q5 同一策略在目标中心视线与完整圆柱口径下的分导弹配对审计。柱上数值来自同一次策略复算。}
\label{fig:q5-robustness}
\end{figure}

工作簿回读后的最大坐标误差、最大单行时长误差和总目标误差分别为 $5.46\times10^{-12}$ m、$3.24\times10^{-11}$ s 和 $1.52\times10^{-11}$ s，全部约束违反量为 0。现有证据证明该策略可执行、可复现，并在受限候选中经连续复算入选；由于未构造全局上界，也未穷尽连续航路，本文不宣称 Q5 全局最优。

\section{跨问题数值验证与敏感性分析}

\subsection{时间离散与连续验根}

图\ref{fig:time-convergence}以 Q1 给定策略和 Q2 优化策略为例，将 $0.50$、$0.25$、$0.10$、$0.05$、$0.02$、$0.01$、$0.005$ s 的均匀计点结果与连续验根值比较。随着步长减小，计点误差总体下降，但误差不必单调，因为端点落在网格中的相位会变化。连续验根把报告精度从计点步长中分离出来；前述漏切触风险仍由括区完备性决定，不能被图中的端点收敛替代。

\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{time_step_convergence.pdf}
\caption{Q1、Q2 均匀计点法相对连续验根的绝对误差。横轴为扫描步长，纵轴为时长误差；连续值作为比较基准。}
...
图\ref{fig:criterion-sensitivity}比较五问的主结果与完整圆柱结果。Q1、Q3 的相对下降分别为 $3.03\%$ 和 $1.35\%$，Q4 下降 $33.88\%$，Q5 总目标下降 $22.09\%$。Q2 两根柱对应两个口径分别优化后的候选，而 Q1、Q3--Q5 是同一策略配对复算，故 Q2 不能被解释为同一决策的前后变化。

\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{criterion_sensitivity.pdf}
\caption{五问对遮蔽判据的敏感性。Q1、Q3--Q5 为同策略配对审计；Q2 为两个口径各自优化后的候选，图中单独标注其语义差异。}
...
\ref{fig:search-quality} & 粗评到连续复算的数值迁移和规模可追踪 & Q3--Q5 diagnostics & 快速评分不可代替终稿值\\
\ref{fig:q5-xy} & 同机策略满足共享水平航向 & Q5 结果表坐标 & 平面投影不含完整高度信息\\
\ref{fig:q5-gantt} & Q5 的逐弹区间、并集和空档可见 & Q5 连续区间 & 空档不等于可达改进量\\
\ref{fig:q5-robustness} & 同一 Q5 决策在圆柱口径下更保守 & Q5 双口径复算 & 不代表风场鲁棒性\\
