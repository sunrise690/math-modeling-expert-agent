...
% Muted, color-blind-friendly editorial palette.
paper = hexColor('#FBFBFA');
ink = hexColor('#23323B');
navy = hexColor('#315F7D');
navyLight = hexColor('#CAD9E2');
smoke = hexColor('#668C7D');
smokeLight = hexColor('#D8E4DE');
amber = hexColor('#C18A42');
darkRed = hexColor('#A95C57');
gray = hexColor('#71808A');
gridColor = hexColor('#DDE3E6');
...
text(axMain, 16.61, 1.668, '← 真目标 T 方向（图窗外）', ...
    'Color', gray, 'FontName', fontName, 'FontSize', 7.0, ...
    'HorizontalAlignment', 'left');
...
eventColors = [amber; amber; navy; amber; darkRed];
eventMarkers = {'d', 'p', '>', 'o', '<'};
...
for k = 1:numel(eventTimes)
    scatter(axTime, eventTimes(k), 0, 32 + 30 * strcmp(eventMarkers{k}, 'p'), ...
...
text(axTime, (entryTime + exitTime) / 2, 0, ...
    ...
    'Color', paper, 'FontName', fontName, 'FontSize', 7.0, ...
...
function drawCriticalPanel(ax, state, mode, panel, ...
        fontName, paper, ink, smoke, smokeLight, accent, gray)
    hold(ax, 'on');
    radius = 10;
    ...
    plot(ax, [xLimits(1), xLimits(2)], [0, 0], '-', ...
        'Color', ink, 'LineWidth', 1.05);
    quiver(ax, xLimits(2) - 4.5, 0, 3.0, 0, 0, ...
        'Color', ink, 'LineWidth', 0.8, 'MaxHeadSize', 0.8);
...
function localNarrative(fig, textPosition, narrative, metricPosition, metric, ...
    ...
    annotation(fig, 'textbox', metricPosition, ...
        ...
        'EdgeColor', 'none', 'BackgroundColor', paper, ...
        'Color', accent, 'FontName', fontName, 'FontSize', 7.0, ...
        'FontWeight', 'bold', 'HorizontalAlignment', 'center', ...
