...
| Q5 最优性 | 53.083939170 s 是否只是启发式偶然结果 | 固定种子、受限路线库与 beam 搜索可复现；连续可行且约束违反为 0 | **仅为高质量可行解，不是全局最优证书** |
...
未发现会使当前策略不可行的证据链断裂。最重要的剩余风险是**搜索最优性而非可行性**：论文可以陈述经验证的时长和方案，但必须保留“固定种子搜索得到的最佳/高质量可行解”措辞，尤其禁止把 Q5 写成全局最优。
