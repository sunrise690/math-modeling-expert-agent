...
Qilintex 将数学建模的阶段化工作流与多人 LaTeX 写作放在同一工作区。成功不是“文件很多”，而是团队能随时知道当前阶段、阻塞风险、证据是否闭环以及下一步应执行什么。
...
- 产品名统一为“Qilintex”。
- 保留中文任务界面与清晰直接的操作文案；旧版蓝色网格视觉已被用户明确否定，不再作为品牌约束。
- 工作台应优先呈现一个当前任务、下一步命令和证据轨迹，避免多重侧栏与密集后台式分区。
- 用户指定 Codex 与 Overleaf 作为交互参考：采用项目化任务线程、过程输出与审核结果的渐进披露，并保持编辑器/PDF 分栏、编译入口和布局切换的稳定性；不复制两者品牌资产。
...
- `apps/client/src/styles.css`：现有蓝色 Swiss 网格视觉体系。
- `C:/Users/1/.codex/skills/cumcm-modeling/`：数模工作流、命令和工件合同。
