...
# This server exposes only the registry's local tools. Starting its own nested
# MATLAB/Origin MCP clients would recurse and is intentionally disabled.
...
mcp = FastMCP(
    ...
    instructions=(
        "Use these deterministic tools for local contest sources, datasets, Python execution, "
...
def execute(name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    if BROKER_DIR and BROKER_TOKEN:
        result = call_file_tool_broker(Path(BROKER_DIR), BROKER_TOKEN, name, arguments)
    else:
        if registry is None:
